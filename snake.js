...
function draw() {
  ...
  text(
    "Score: " +
      "000000000".slice(0, -Score.value.toString().length).concat(Score.value),
    30,
    750,
    655,
    100
...
class Point {
  /* The Point class will be used both for snake and food */
...
class Food {
  constructor() {
    ...
    this.y =
      snake.head.height * Math.round((Math.random() * WINDOW_HEIGHT) / 50);
  ...
  ifCollisionWith(player) {
    let hasCollided =
      Math.abs(this.x - player.head.x) === 0 &&
      Math.abs(this.y - player.head.y) === 0;
...
class Snake {
  ...
  draw(point = this.head) {
    ...
    if (point.parent) {
      ...
      if (
        Math.abs(point.x - this.head.x) === 0 &&
        Math.abs(point.y - this.head.y) === 0
      ) {
...
class Events {
  ...
  handle() {
    let isConfiguredInput = Object.keys(this.input)
